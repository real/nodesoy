...
var closureLibrary = require('unreal-closure-library')
...
/**
 * An abstract API over a nodesoy VM context.
 *
 * nodesoy operates by creating a VM sandbox, and loading the soy functions into
 * that sandbox. If you use nodesoy's i18n features, you may have multiple sandboxes,
 * one for each locale.
 *
 * @param {string} name
...
SoyVmContext.prototype.get = function (templateName) {
  if (!this._options.loadCompiledTemplates) throw new Error('nodesoy: Cannot load template, try with `loadCompiledTemplates: true`.')

  if (!this._templateCache[templateName]) {
    var template
    try {
      template = vm.runInContext(templateName, this.getContext(), 'nodesoy.vm')
    } catch (e) {
      // Fallthrough
    }

    if (!template) throw new Error('nodesoy: Unknown template [' + templateName + ']')
...
SoyVmContext.prototype.loadCompiledTemplateFiles = function (files, callback) {
  ...
  if (self._contextInitialized) {
    result = Q.fcall(function () {
      vm.runInContext(RESET_DELTEMPLATE_REGISTRY_CODE, self.getContext(), 'nodesoy-reset.')
    })
  } else {
