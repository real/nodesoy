...
/**
 * The main public API of nodesoy.
 * @constructor
 */
...
/**
 * Sets options which affect how nodesoy operates.
 * @param {{
...
SoyCompiler.prototype._compileTemplateFilesAndEmit = function (inputDir, files, emitter) {
  ...
  function onExit(exitCode) {
    ...
    if (exitCode != 0) {
      ...
      terminated = true
      console.error('nodesoy: Compile error\n', stderr)
...
SoyCompiler.prototype._finalizeCompileTemplates = function (outputDir, emitter) {
  ...
  if (this._options.eraseTemporaryFiles) {
    exec('rm -r \'' + outputDir + '\'', {}, function (err, stdout, stderr) {
      ...
      if (err) console.error('nodesoy: Error deleting temporary files', err)
    })
  }
};
  ...
  files.forEach(function (file) {
    ...
    try {
      ...
      fs.watchFile(file, {}, function (event, filename) {
        ...
        console.log('nodesoy: Recompiling templates due to change in [%s]', file)
        this._watches[file] = now
        fn.call(this)
      }.bind(this))
    } catch (e) {
      console.warn('nodesoy: Error watching ' + file, e)
    }
  }, this)
}
...
SoyCompiler.prototype._postCompileProcess = function (outputDir, files, vmType, callback) {
  ...
  try {
    ...
    console.warn('nodesoy: Error concatenating files', e)
  }
...
function logErrorOrDone(err, res) {
  if (err) console.error('nodesoy:', err)
  else console.log('nodesoy: Done')
}
